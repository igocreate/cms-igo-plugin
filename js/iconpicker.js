...
$.fn.iconpicker = function() {
  ...
  element.parent().on('click', '.icons > a', function() {
    const val = $(this).attr('title');
    element.val(val);
    element.popover('hide');
    const selected = element.parent().find('i.selected');
    selected.removeClass();
    selected.addClass('selected ' + val);
    return false;
  });

  element.parent().on('keyup', '.filter input', function() {
    const q = $(this).val().toLowerCase();
    element.parent().find('.icons a').each(function() {
      const a = $(this);
      a.toggle(!q || a.attr('title').indexOf(q) > -1);
    });
  });
};
